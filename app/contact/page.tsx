...
import { MapPin, Phone, Mail, Clock, Facebook, ExternalLink, Accessibility, Car } from "lucide-react";
      ...
      <section className="bg-[var(--brand-cream)] py-16 md:py-24" aria-label="Contact details">
        <div className="max-w-[1400px] mx-auto px-5 md:px-8 grid lg:grid-cols-12 gap-10">
          <div className="lg:col-span-5 space-y-8">
            ...
            <div>
              ...
              <a href={`tel:${business.phoneTel}`} className="font-display text-2xl md:text-3xl font-bold text-[var(--brand-deep)] hover:text-[var(--brand-ember)] transition-colors phone">
                {business.phone}
              </a>
            </div>
            ...
            <div>
              ...
              <a href={`mailto:${business.email}`} className="font-display text-xl md:text-2xl font-bold text-[var(--brand-deep)] hover:text-[var(--brand-ember)] transition-colors break-all">
                {business.email}
              </a>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2.5 text-[14px] text-[var(--brand-deep)] opacity-85">
                <span className="inline-flex items-center gap-2">
                  <Accessibility className="w-4 h-4 text-[var(--brand-saffron)]" />
                  Wheelchair accessible
                </span>
                <span className="inline-flex items-center gap-2">
                  <Car className="w-4 h-4 text-[var(--brand-saffron)]" />
                  Private lot parking
                </span>
                <span className="inline-flex items-center gap-2 text-[12px] text-[var(--brand-muted)]">
                  Accepts credit cards, Apple Pay & bike parking
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              {business.social.map((s) => (
                <a key={s.platform} href={s.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border-2 border-[var(--brand-deep)] text-[var(--brand-deep)] hover:bg-[var(--brand-deep)] hover:text-[var(--brand-cream-bright)] font-semibold text-sm transition-all">
        ...
        <div className="max-w-[1400px] mx-auto px-5 md:px-8 mt-20">
          <div className="text-center mb-10">
            ...
            <h2 className="mt-2 font-display headline-medium text-[var(--brand-deep)]">
              NorthPark Produce across San Diego.
            </h2>
          </div>
            ...
            {business.allLocations.map((loc) => (
              <div
                ...
                className={`p-6 rounded-2xl border ${
                  loc.current
                    ? "border-[var(--brand-ember)] bg-[var(--brand-cream-bright)]"
                    : "border-[rgba(140,123,107,0.2)] bg-[var(--brand-cream-bright)]"
                }`}
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  <h3 className="font-display text-lg font-bold text-[var(--brand-deep)] leading-tight">
                    {loc.name}
                  </h3>
                  {loc.current && (
                    <span className="text-[10px] uppercase tracking-[0.18em] px-2 py-0.5 rounded-full bg-[var(--brand-ember)] text-[var(--brand-deep)] font-bold mono">
                      You&rsquo;re here
                    </span>
                  )}
                </div>
                <p className="text-[13px] text-[var(--brand-muted)] mb-2 leading-relaxed">
                  {loc.address}
                </p>
                <a
                  href={`tel:${loc.phone.replace(/[^0-9+]/g, "")}`}
                  className="text-[13px] font-medium text-[var(--brand-ember)] hover:text-[var(--brand-ember-light)] phone"
                >
                  {loc.phone}
                </a>
              </div>
